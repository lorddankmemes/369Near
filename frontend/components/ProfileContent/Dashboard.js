                ...
                <tbody className='text-xs' key={index}>
                    <tr className="border-b w-full">
                        ...
                        <td scope="col" className='flex gap-x-8 pr-6 py-4 justify-center'>
                            <span><img className="market-size z-10" src={data.transaction_of_collectible.ipfs_media_path}/></span>
                            <span className='block pt-3 truncate'>{data.transaction_of_collectible.collectible_name}</span>
                        </td>
                        <td scope="col" className="px-10 py-4 whitespace-nowrap text-center">{Number((Math.round(data.unit_price / 1000000000000)).toFixed(3))}</td>
                        <td scope="col" className="py-4 whitespace-nowrap text-center">{data.quantity}</td>
                        {/* <td><p className='truncate'>{data.activity_by}</p></td> */}
                        <td scope="col" className='flex gap-x-8 pr-6 justify-center py-4'>
                            <span><img className="block market-size z-10" src={data.transaction_of_collectible.ipfs_media_path}/></span>
                            <span className='block pt-3 w-[64px] lg:w-[100px] truncate'>{data.activity_by}</span>
                        </td>
                        <td scope="col" className="px-6 py-4 whitespace-nowrap">{data.transfer_to_username}</td>
                        <td scope="col" className="pl-10 py-4 whitespace-nowrap text-right">{moment(data.activity_datetime).fromNow()}</td>
                    </tr>
                </tbody>
                ))}
                </table>
